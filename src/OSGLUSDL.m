...
static SDL_mutex *render_lock = NULL;
	...
	ui4r bottom, ui4r right)
{
	...
	if (SDL_LockMutex(render_lock)) return;
...
{
	if (*TheSoundBuffer != kCenterSound)
		QuietEnds();
...
LOCALFUNC blnr ReCreateMainWindow(void)
{
	...
	*/
	...
	if (SDL_LockMutex(render_lock)) return falseblnr;
...
static void disable_SDL2_macosx_menu_bar_keyboard_shortcuts() {
...
int main(int argc, char **argv)
{
	...
	ZapOSGLUVars();
	if (InitOSGLU() && (render_lock = SDL_CreateMutex())) {
		__block volatile int f = 0;
